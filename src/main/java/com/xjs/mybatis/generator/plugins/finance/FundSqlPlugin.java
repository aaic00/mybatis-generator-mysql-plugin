...
public class FundSqlPlugin extends SqlPlugin {
  ...
  public boolean clientGenerated(final Interface interfaze, final TopLevelClass topLevelClass,
    ...
    if (PropertiesUtils.isTrue(introspectedTable, FundSqlPlugin.TABLE_ENABLE_SELECT_RECENT_REAL)) {
      this.addMethodSelect(interfaze, introspectedTable, entityType,
          FundSqlPlugin.TABLE_ENABLE_SELECT_RECENT_REAL,
          new Parameter(FullyQualifiedJavaType.getStringInstance(), "code"));
    }

    if (PropertiesUtils.isTrue(introspectedTable, FundSqlPlugin.TABLE_ENABLE_DELETE_ESTIMATE)) {
      this.addMethodDelete(interfaze, introspectedTable, entityType,
          FundSqlPlugin.TABLE_ENABLE_DELETE_ESTIMATE,
          new Parameter(FullyQualifiedJavaType.getStringInstance(), "code"));
    }
